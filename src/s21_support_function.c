...
int s21_minor_matrix(matrix_t *A, int row_skip, int col_skip, matrix_t *result) {
    ...
    if (s21_check_matrix(A) == INCORRECT_MATRIX) {
        ...
        int i_minor = 0;
        int j_minor = 0;
        // Создание матрицы под результат вычисления (порядок матрицы на 1 меньше)
        s21_create_matrix(A->rows - 1, A->columns - 1, result);

        // Перебор строк входящей матрицы А
        for (int i_matrix = 0; i_matrix < A->rows; ++i_matrix) {
            // Если рассматриваемый индекс строки и индекс элемента, для которого
            // вычисляется минор совпадают, переход к следующей строке
